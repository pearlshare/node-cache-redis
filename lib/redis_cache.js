 ...
 * @param    {boolean}  options.enabled - Whether to enable the cache
...
RedisCache.prototype.getTtl = function (ttlInSeconds) {
  // If ttl is a valid number use it
  if (typeof ttlInSeconds === "number" && !isNaN(ttlInSeconds)) {
    return ttlInSeconds;
  }
  // else if ttl is present but invalid use 0 (no cache)
...
RedisCache.prototype.set = function (key, value, ttlInSeconds) {
  if (this.getTtl(ttlInSeconds) === 0 || !this.enabled) return Promise.resolve();
